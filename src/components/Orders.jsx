...
function Orders() {
  ...
  const [selectedPickupLocation, setSelectedPickupLocation] = useState('SATVIK RASS');
                ...
                {orders.map((order, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <button 
                        onClick={async (event) => {
                          const res = await handleShipment(order.razorpayOrderId);
                          if (res) {
                            updateOrderStatus(order.razorpayOrderId);
                          }
                        }} 
                        disabled={order.status !== 'PAID'}
                        className={`forward-order-btn ${order.status !== 'PAID' ? 'disabled' : ''}`}
                      >
                        Make Forward Order
                      </button>
                    </td>
